...
%%% p15.m Particle Filter (SIRS, OP), sin map (Ex. 1.3)
%% setup
...
for j=1:J   
    ...
    Sig=inv(inv(sigma^2)+H'*inv(gamma^2)*H);% optimal proposal covariance
    em=Sig*(inv(sigma^2)*alpha*sin(U(j,:))+H'*inv(gamma^2)*y(j));% proposal mean
...
title('Particle Filter (Optimal), Ex. 1.3');
